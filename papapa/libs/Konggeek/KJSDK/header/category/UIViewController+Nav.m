...
#define BAR_BUTTON_ITEM_FONT 15
...
- (void)barButtonItemTitle:(NSString *)itemTitle position:(NSString *)position {
    ...
    [button setTitleColor:[UIColor colorWithHexString:BAR_BUTTON_ITEM_TITLECOLOR] forState:UIControlStateNormal];
...
/// LeftBarButtonItem itemTitle:标题 titleColor:标题颜色 position：left or right
- (void)barButtonItemTitle:(NSString *)itemTitle titleColor:(NSString *)titleColor position:(NSString *)position {
    UIButton *button = [[UIButton alloc] init];
    CGFloat width = [itemTitle widthWithFont:BAR_BUTTON_ITEM_FONT];
    button.frame = RECT(0, 0, width, 40);
    button.titleLabel.font = FONT(BAR_BUTTON_ITEM_FONT);
